...
当前运行主链路已经统一切到内置托管的 `aria2`：

- 主进程负责托管 `aria2c` 进程与 RPC 连接
- 下载任务通过 `aria2` JSON-RPC 创建、同步和清理
- renderer 只负责展示与交互，不直接处理下载逻辑

仓库里仍保留了一些 qBittorrent / BT adapter 相关代码，但它们不是当前生效主链路。
